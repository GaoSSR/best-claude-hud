...
impl ConfigLoader {
    ...
    /// Get the themes directory path (~/.claude/best-claude-hud/themes/)
    pub fn get_themes_path() -> PathBuf {
        if let Some(home) = dirs::home_dir() {
            home.join(".claude").join("best-claude-hud").join("themes")
        } else {
            PathBuf::from(".claude/best-claude-hud/themes")
        }
    }
...
impl Config {
    ...
    /// Get the default config file path (~/.claude/best-claude-hud/config.toml)
    fn get_config_path() -> PathBuf {
        if let Some(home) = dirs::home_dir() {
            ...
            PathBuf::from(".claude/best-claude-hud/config.toml")
        }
    }
